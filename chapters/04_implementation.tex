...
Figure~\ref{fig:parsed-plan-hash-join-cutout} shows a representation for a Hash Join and its children, directly taken from the ZeroShot benchmark data~\parencite{zeroshot}.
Here, we can gather data like cardinalities and filter predicates (\texttt{filter\_columns}) from \texttt{plan\_parameters}.
...
Scan operators are mapped to \texttt{tablescan}, join operators to \texttt{join}, sort nodes to \texttt{sort}, and aggregate nodes to \texttt{groupby}.
The PostgreSQL-specific temporary operators hash and materialize are mapped to the abstract operator \texttt{temp}.

Every converted node additionally stores a copy of the original \texttt{plan\_parameters} under a dedicated \texttt{pg} payload.
This payload is used for feature extraction.

T3 also requires database schema information. However, the ZeroShot data does not provide this information, so a minimal placeholder schema is used instead.

The pipeline assignment is stored in the \texttt{analyzePlanPipelines} list of the converted plan.
For every pipeline, we derive its duration from the actual node timings in \texttt{plan\_parameters}.
...
(Section~\ref{sec:evaluation_ablation}). There, we also analyze the derivation and its impact on label precision.


\section{Feature Vector Construction}

We use \texttt{pg\_features.py} to construct the feature vector for each pipeline after the pipeline decomposition, using the \texttt{pg} payload.
...
\begin{enumerate}
  ...
  For every operator that occurs in the ZeroShot data, we assign a slice in the feature vector.
  If multiple operators of the same type occur in the pipeline, we sum the features over all occurrences.
  In Figure~\ref{fig:pg-feature-vector-snippet}, the excerpt of the operator-type block for the Nested Loop Join operator is depicted.
  ...
  In the ZeroShot data, they are represented as a structured predicate tree in the \texttt{filter\_columns} field~\parencite{zeroshot}.

  Finally, we include the operator count ``\texttt{const}'', which exists for every operator type in both our and the T3 (Umbra) implementation~\parencite{t3}.
\end{enumerate}

The full feature vector has a fixed dimensionality of 153 entries: 23 pipeline-level features and 130 operator-type features.
We zero empty entries to maintain a constant feature vector size.
...
\begin{figure}[htpb]
  ...
  \caption[Tree Cutout with Hash Join from ZeroShot Data]{Tree cutout of one \texttt{parsed\_plan} containing a Hash Join from \path{credit/workload_100k_s1_c8220}. Child relationships are shown by arrows,
   and each node box contains selected \texttt{plan\_parameters} of that node.}
