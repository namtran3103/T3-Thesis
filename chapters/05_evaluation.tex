...
Despite the increased variation in the average and p90 values, we conclude that the model is able to generalize across unseen database instances.
...
Thus, it is still is a good starting point.
...
\begin{figure}[htpb]
  ...
  \caption[Cardinality Mode Comparison]{Q-Error (p50, p90, and average) over all queries under three cardinality settings: trained and evaluated on exact cardinalities, trained on exact but evaluated on estimated cardinalities, and trained and evaluated on estimated cardinalities.}\label{fig:card_comparison}
\end{figure}


\paragraph{Query vs. Pipeline vs. Tuple Targets.}
...
that can be extracted from PostgreSQL's \texttt{EXPLAIN ANALYZE} output in the ZeroShot dataset~\parencite{postgresql_docs_query_path}.
...
However, PostgreSQL uses the Volcano iterator model~\parencite{graefe1994},
...
\texttt{EXPLAIN ANALYZE} records the total wall-clock interval for each node,
so both the join (P1) and the scan (P0) receive overlapping time intervals —
even though they belong to different derived pipelines.

When these node durations are aggregated into per-pipeline times and then summed,
the overlapping portion is counted twice,
so the sum of pipeline runtimes exceeds the actual query execution time.
A model trained to predict query runtime as the sum of pipeline runtimes
will therefore over-predict, increasing Q-Error.

As a concrete example, in plan 2 of \texttt{tpch/workload\_100k\_s1\_c8220},
pipeline P0 (table scan) is recorded from $t = 946$ms to $t = 2{,}156$ms,
giving a span of 1{,}210ms.
Further, Pipeline P1 (table scan, join) is recorded from $t = 0$ms to $t = 2{,}185$ms,
giving a span of 2{,}185ms.
Thus, P0 is entirely nested inside P1's time window,
so the 1{,}210ms during which both pipelines are simultaneously open
is counted once in P0 and again in P1.
When summed, the two spans alone already yield 3{,}395ms,
yet the actual query execution time is only 2{,}239ms —
an overcount of 52\%.
...
Subsequently, with a narrower range and per-pipeline targets that are already unreliable,
the per-tuple decomposition provides no accuracy benefit and introduces additional inaccuracies instead.
...
\begin{figure}[htpb]
  ...
  \caption[Query Running Time Distribution]{Distribution of all query running times of the ZeroShot dataset.}\label{fig:query_runtime_hist}
\end{figure}
