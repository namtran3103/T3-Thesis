...
This is done by training the model on all databases except the one being tested.
...
They fall in the same range as the ZeroShot model~\parencite{zeroshot}.
...
We scope this finding to the ZeroShot query patterns~\parencite{zeroshot}.
Generalization to even more structurally diverse queries remains untested and is addressed in the future work (Chapter~\ref{chapter:future_work}).
...
T3 and the ZeroShot model~\parencite{zeroshot} offer very good generalization performance across unseen databases.
Because T3 uses different queries for generalization testing, we cannot directly compare the generalization performance of our model to theirs.
However, Rieger \& Neumann compared T3 and ZeroShot on the JOB-Benchmark and reported excellent performance for both models~\parencite{t3}.
...
If the missing queries had a very high Q-Error, the p50 value of our model would increase and thus using the current p50 value would be misleading.
...
There, our model performs worse than the ZeroShot and T3 (Umbra) models on the JOB-Benchmark.
...
Although we only have 70\% of the JOB-queries available for the calculation on our model, we still see a notable trend. 
...
The comparison above used actual cardinalities to match the setup reported by Rieger \& Neumann~\parencite{t3}.
Wehrstein et al. instead report their results using estimated cardinalities~\parencite{jobcomplex}, so we switch to estimated cardinalities here to ensure a fair comparison.
They ran the JOB-Benchmark on their T3-based model and reported a median Q-Error of 4.07.
Like before, they had the full 113 queries at disposal, so we again use the 57th-best (p75) Q-Error value for our model.

Although we are in the worst-case-scenario, our model clearly outperforms the model of Wehrstein et al., as Figure~\ref{fig:comp_wehrstein} shows.
A key difference between the models is the feature vector: only our model uses a fully PostgreSQL-native feature vector.
...
Finally, PostgreSQL's untrained, built-in cost estimator performs worst by a very large margin at a 57th-best Q-Error of over 1000, stating the need for learned approaches.
...
Figure~\ref{fig:card_comparison} shows prediction accuracy under three cardinality conditions, aggregated over all queries.
...
When, for the models trained on actual cardinalities, we switch to estimated cardinalities for inference, accuracy degrades.
...
a lower average Q-Error than the model trained on exact cardinalities and evaluated on estimated ones.
Thus, this indicates that the model can somewhat learn to account for the systematic biases of the cardinality estimator~\parencite{leis2018job}.
However, the p90 Q-Error stays at a similar, high level in both estimated-cardinality settings, showing that a small number of heavy outliers remain.
...
Figure~\ref{fig:query_pipeline_tuple} compares the Q-Error across three prediction granularities:
per-query, per-pipeline, and per-tuple.
This copies the evaluation of Rieger and Neumann~\parencite{t3},
...
In T3, the per-tuple runtime distribution spans from $10^{-15}$\,s to $1$\,s -- a range of fifteen orders of magnitude~\parencite{t3}.
Rieger and Neumann identify this wide range as the primary motivation for the per-tuple approach.
...
the per-tuple decomposition provides no accuracy benefit.
