...
The model generalizes reliably across unseen PostgreSQL database instances, with a median Q-Error below 1.33 across all 20 ZeroShot databases.
...
Consequently, this confirms that a PostgreSQL-native feature vector is beneficial.
...
Subsequently, the per-tuple approach could be re-evaluated, especially with future work concerning the pipeline timing issue.
A purely query-level feature vector -- bypassing pipeline decomposition -- could further sidestep the timing issue while retaining the speed advantage of LightGBM.
Prediction latency should also be benchmarked and potentially improved through native code compilation using the \texttt{lleaves} framework~\parencite{t3}.
